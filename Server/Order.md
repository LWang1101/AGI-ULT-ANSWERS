Linux系统的文本编辑器和windows还是不太一样，在这里做个简单的记录。

^G 求助         ^O 写入         ^W 搜索         ^K 剪切文字     ^J 对齐         ^C 游标位置     M-U 撤销
^X 离开         ^R 读档         ^\ 替换         ^U 还原剪切     ^T 拼写检查     ^_ 跳行         M-E 重做
上面是我文本编辑器中的操作提示，其中有两个键需要注意一下'^'和'M-'。

'^'对应的就是ctrl键。

'M-'对应的是alt键。

所以“离开”的操作为'ctrl+x'，“撤销”的操作为'alt+u'。

首先介绍一下Ubuntu下各个目录的一般作用：
